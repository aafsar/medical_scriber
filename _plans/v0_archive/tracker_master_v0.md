...
| Phase 6: End-to-End Testing | DONE | ElevenLabs wins accuracy (95% vs 88%) + diarization; Deepgram wins latency (26.6s vs 45.0s). No code bugs. |
...
**Status:** DONE. Details → [`_plans/phase6/tracker_phase6.md`](phase6/tracker_phase6.md)

---

## Provider Comparison Results

*From `test_results/2026-03-07_211700/test_summary.md`. Full detail in Phase 6 tracker.*
...
- **Deepgram** over-segments utterances 4-5x (82-106 Doctor utterances vs expected ~20). Missed multi-word medical terms (Kellgren-Lawrence, hemoglobin A1c, gastroesophageal reflux).
- **ElevenLabs** utterance counts closely match script structure (20-22 per speaker). Benefited from `keyterms` parameter for medical vocabulary. Only common miss: "microalbuminuria".
- Both providers missed "diastolic" in the cardio script.
- Note generation latency similar (~12-14s) regardless of provider.
...
| 5 | 2026-02-21 | No database for v0 | Download notes as files; simplicity over persistence |
...
| 9 | 2026-03-07 | Build automated test runner | Separate Streamlit page + pure logic module. One button runs both providers, auto-saves outputs, generates comparison reports with term accuracy metrics. Recording remains manual. |
...
| `compare.py` | Test comparison logic: term accuracy, diarization stats, reports | DONE |
| `pages/test_runner.py` | Automated test UI: dual-provider pipeline, metrics, file saving | DONE |
